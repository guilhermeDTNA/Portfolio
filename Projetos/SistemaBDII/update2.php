...
//$num_alunos = addslashes($_POST['num_alunos']);
    ...
    if ($id_professor==''||$nome==''||$mensalidade=='') {
        echo "<script type=text/javascript>alert('Operação NÃO realizada! Alguns dados estão inválidos');window.location='alterarDados.php?tipo=curso'</script>";
    }
    ...
    if ($id_diretor==''||$nome==''||$valor_despesa=='') {
        echo "<script type=text/javascript>alert('Operação NÃO realizada! Alguns dados estão inválidos');window.location='alterarDados.php?tipo=curso'</script>";
    }
    ...
    if ($id_aluno==''||$id_curso=='') {
        echo "<script type=text/javascript>alert('Operação NÃO realizada! Alguns dados estão inválidos');window.location='alterarDados.php?tipo=matricula'</script>";
    }
...
//$senha = addslashes($_POST['senha']);
    ...
    if ($usuario==''||$_POST['senha']=='') {
        echo "<script type=text/javascript>alert('Operação NÃO realizada! Alguns dados estão inválidos');window.location='alterarDados.php?tipo=diretor'</script>";
    }
...
if ($nome==''||$sobrenome==''||$email==''||$data_nasc==''||$rua==''||$numero==''||$cidade==''||$estado=='') {
    echo "<script type=text/javascript>alert('Operação NÃO realizada! Alguns dados estão inválidos');window.location='alterarDados.php?tipo='".$_POST['oTipo']."'</script>";
}
...
// $stmt->bindParam($data_nasc, addslashes($_REQUEST['data_nasc']));
        ...
        if ($_SESSION['usuario']==$usuario) {
            echo "<script type=text/javascript>alert('Operação realizada com sucesso! Realize o login novamente');window.location='logout.php?'</script>";
        }
    }
